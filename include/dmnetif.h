...
/**
 * Public API for the dmnetif module.
 *
 * Functions are declared with the dmod_dmnetif_api(...) macro - dmod's
 ...
 *
 * Definitions in src/dmnetif.c use the matching
 * dmod_dmnetif_api_declaration(...) macro - a plain C function
 ...
 * src/dmnetif.c with your module's real API.
 */

/* Opaque handle - the real struct is defined in src/dmnetif.c */
typedef struct dmnetif* dmnetif_t;

/**
 * Create a new dmnetif instance.
 *
 * @return A valid handle on success, or NULL on allocation failure.
 */
dmod_dmnetif_api(1.0, dmnetif_t, _create, ( void ));

/**
 * Destroy an instance created by dmnetif_create(). Safe to call with
 * NULL.
 */
dmod_dmnetif_api(1.0, void, _destroy, ( dmnetif_t handle ));
 ...
 */
dmod_dmnetif_api(1.0, bool, _is_valid, ( dmnetif_t handle ));
